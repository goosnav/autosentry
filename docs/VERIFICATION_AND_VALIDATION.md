...
| FR-3 | T | L4 | `eval_detection.py` precision/recall on labeled set; track-ID continuity test. | M1 | ◐ `test_tracking` continuity + `test_eval_detection` metrics ☑; approach trigger now measures speed over a recent window (per-bbox `history_ts`), so a loiter-then-sprint can't hide under the lifetime average — `test_triggers` proves it fires where the old average would not ☑; P/R gate needs labeled benchmark |
...
| IR-1..4 | I/T | L3 | Inspect each seam against ICD-1..7; codec round-trip tests. | M0–M3 | ◐ IR-2 (ICD-2 transport) COBS/CRC round-trip + corruption ☑, gateway-firmware counterpart implemented (`AUTOSENTRY_ROLE_GATEWAY`) + golden serial vectors pinned hub-side (`test_wire_vectors`), node `pio test` pending; IR-3 (ICD-3 air) payload codec round-trips ☑; IR-4 scaffolded |
